...
impl ProgramContext {
    ...
    ///
    /// Reads every symbol name from oxc's `Scoping` so that UID generation avoids
    /// name conflicts with any binding in the file (matching Babel's
    /// `generateUid()`, which checks all scopes).
